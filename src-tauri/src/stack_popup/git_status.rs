...
fn stack_git_commit_files(
    ...
    let status_output = git_stdout_bytes(&repo_root, &commit_files_git_args(&commit_hash, true))?
        .unwrap_or_default();
    let numstat_output = git_stdout_bytes(&repo_root, &commit_files_git_args(&commit_hash, false))?
        .unwrap_or_default();
...
fn commit_files_git_args<'a>(commit_hash: &'a str, name_status: bool) -> Vec<&'a str> {
...
mod tests {
    ...
    fn git_remote_url_normalizer_handles_common_browser_remotes() {
        ...
        assert_eq!(
            normalize_git_remote_url("https://bitbucket.org/acme/repo.git", "release/v1")
                .as_deref(),
    ...
    fn git_branch_path_encoder_preserves_slashes_and_escapes_specials() {
        assert_eq!(
            encode_git_branch_path("feature/x").as_deref(),
            Some("feature/x")
        );
        assert_eq!(
            encode_git_branch_path("bug fix").as_deref(),
            Some("bug%20fix")
        );
        assert_eq!(
            encode_git_branch_path("release#1").as_deref(),
            Some("release%231")
        );
    ...
    #[test]
    fn commit_files_git_args_use_first_parent_and_root_mode() {
        let args = super::commit_files_git_args("abc123def4567890abc123def4567890abc123de", true);
        assert_eq!(
            args,
            vec![
                "show",
        ...
        let numstat_args =
            super::commit_files_git_args("abc123def4567890abc123def4567890abc123de", false);
        assert_eq!(
            numstat_args,
            vec![
                "show",
    ...
    #[test]
    fn commit_files_backend_includes_merge_commit_changes_against_first_parent() {
        if Command::new("git").arg("--version").output().is_err() {
            return;
        }
        let repo_root = std::env::temp_dir().join(format!(
            "jasonshell-git-commit-files-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
                .as_nanos()
        ...
        let root_file = repo_root.join("root.txt");
        std::fs::write(&root_file, b"root\n").unwrap();
        let add_root = Command::new("git")
            .arg("-C")
            .arg(&repo_root)
            .arg("add")
            .arg("root.txt")
        ...
        let root_commit = Command::new("git")
            ...
            .unwrap();
        assert!(
            root_commit.status.success(),
            "root commit failed: {:?}",
            root_commit
        );
        let root_hash = String::from_utf8_lossy(
            &Command::new("git")
                .arg("-C")
                .arg(&repo_root)
                .arg("rev-parse")
        ...
        .trim()
        .to_string();
        let root_result = super::stack_git_commit_files(super::StackGitCommitFilesRequest {
            folder_path: repo_root.to_string_lossy().to_string(),
            commit_hash: root_hash.clone(),
        })
        .unwrap();
        assert_eq!(
            root_result
        ...
        assert_eq!(root_result.files[0].additions, 1);
        assert_eq!(root_result.files[0].deletions, 0);

        let side_branch = Command::new("git")
            .arg("-C")
            .arg(&repo_root)
            .arg("checkout")
            .arg("-b")
            ...
            .unwrap();
        assert!(
            side_branch.status.success(),
            "git checkout -b side failed: {:?}",
            side_branch
        );
        let branch_file = repo_root.join("branch.txt");
        std::fs::write(&branch_file, b"branch\n").unwrap();
        let add_branch = Command::new("git")
            .arg("-C")
            .arg(&repo_root)
            .arg("add")
            .arg("branch.txt")
            ...
            .unwrap();
        assert!(
            add_branch.status.success(),
            "git add branch.txt failed: {:?}",
            add_branch
        );
        let branch_commit = Command::new("git")
            .arg("-C")
            .arg(&repo_root)
            .arg("commit")
            .arg("-m")
        ...
        let checkout_mainline = Command::new("git")
            .arg("-C")
            .arg(&repo_root)
            .arg("checkout")
            .arg(&root_hash)
            ...
            .unwrap();
        assert!(
            checkout_mainline.status.success(),
            "git checkout root failed: {:?}",
            checkout_mainline
        );
        let create_mainline = Command::new("git")
            .arg("-C")
            .arg(&repo_root)
            .arg("checkout")
            .arg("-b")
            ...
            .unwrap();
        assert!(
            create_mainline.status.success(),
            "git checkout -b mainline failed: {:?}",
            create_mainline
        );
        std::fs::write(&root_file, b"root updated\n").unwrap();
        let add_mainline = Command::new("git")
            .arg("-C")
            .arg(&repo_root)
            .arg("add")
            .arg("root.txt")
            ...
            .unwrap();
        assert!(
            add_mainline.status.success(),
            "git add root.txt on mainline failed: {:?}",
            add_mainline
        );
        ...
        let merge = Command::new("git")
            ...
            .output()
            .unwrap();
        assert!(merge.status.success(), "merge failed: {:?}", merge);

        let merge_hash = String::from_utf8_lossy(
            &Command::new("git")
                .arg("-C")
                .arg(&repo_root)
                .arg("rev-parse")
        ...
        .trim()
        .to_string();
        let with_first_parent = Command::new("git")
            .arg("-C")
            .arg(&repo_root)
            .arg("show")
            .arg("--format=")
            ...
            .unwrap();
        assert!(
            with_first_parent.status.success(),
            "git show first-parent failed: {:?}",
            with_first_parent
        );
        let with_first_parent_text = String::from_utf8_lossy(&with_first_parent.stdout);
        assert!(!with_first_parent_text.trim().is_empty());
        assert!(with_first_parent_text.contains("branch.txt"));
        assert!(with_first_parent_text.contains("+branch"));
        assert!(!with_first_parent_text.contains(&merge_hash));

        let diff = super::stack_git_commit_file_diff(super::StackGitCommitFileDiffRequest {
            folder_path: repo_root.to_string_lossy().to_string(),
            commit_hash: merge_hash.clone(),
            path: "branch.txt".to_string(),
        })
        .unwrap();
        assert_eq!(diff.commit_hash, merge_hash);
        assert_eq!(diff.path, "branch.txt");
        assert!(diff.content.contains("branch.txt"));
        assert!(diff.content.contains("+branch"));

        let request = super::StackGitCommitFilesRequest {
            folder_path: repo_root.to_string_lossy().to_string(),
            commit_hash: merge_hash.clone(),
        };
        let result = super::stack_git_commit_files(request).unwrap();
        assert!(result
            .repository_root
            .ends_with(&*repo_root.to_string_lossy()));
        assert_eq!(result.commit_hash, merge_hash);
        ...
        let paths = result
            ...
            .map(|file| file.relative_path.as_str())
            .collect::<Vec<_>>();
        assert_eq!(paths, vec!["branch.txt"]);
        let _ = std::fs::remove_dir_all(&repo_root);
    }

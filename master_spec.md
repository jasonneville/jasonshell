...
- Search accessibility/control P4, 2026-05-03, superseded for Windows-key suppression on 2026-05-06: `TopBar.svelte` and centered `SearchPanelSurface.svelte` now render keyboard-accessible clear buttons only when their visible query/draft is non-empty. Clear actions route through the same immediate query reset path (`publishImmediateSearchInputState('')`, `startImmediateSearchQueryExecution(request)`, and centered `queueQueryEmit('')`) so latest-query sequence gates reject stale provider responses; they do not call providers, native show APIs, or panel publish paths directly. `src/features/search/searchUxState.ts` exports `clearLatestSearchQuery(...)` for explicit stale-response invalidation. Windows-only `src-tauri/src/windows_key_hook.rs` installs an app-lifecycle-scoped low-level keyboard hook after startup warm begins. The hook passes Left/Right Windows-key down events through so native `Win+R`, `Win+D`, `Win+E`, and `Win+L` chords can reach Windows; bare final Windows-key release emits centered search and is suppressed so Start should not activate. Stray/duplicate Windows-key releases still suppress, left/right Windows-key overlap is tracked independently, chorded final releases pass through, unavailable hook state fails closed for Windows-key events, install failure aborts setup, no keystrokes are logged or persisted, only `search:open-centered` is emitted to `top-bar`, and uninstall remains app-exit cleanup.
...
- Startup sequence:
  ...
  5. On Windows, call `windows_key_hook::install_windows_key_hook(app.handle().clone())` to suppress Start Menu Windows-key events and bind bare Windows-key taps to centered search while leaving non-Windows chord keys to the OS. This hook is required; install failure returns a setup error instead of starting JasonShell in a degraded mode.
...
- `.setup()` creates windows first, starts index warming second, installs the required Windows-key suppression hook, then activates AppBars on Windows. This order matters: AppBar activation needs native HWNDs; search warming must not block surface creation; the Windows-key hook must be active before the shell is considered running; non-Windows fallback only shows top/bottom windows and leaves AppBar/runtime metric commands unsupported.
...
- `src-tauri/src/windows_key_hook.rs` owns the Windows-only low-level keyboard hook, tap/chord classifier, idempotent install/uninstall, Start Menu suppression fail-closed behavior, and `search:open-centered` event emission to `crate::shell_windows::TOP_BAR_LABEL`; it must never target `search-panel` directly or log/persist key data.
...
- `search:open-centered`: emitted by `windows_key_hook.rs` to `top-bar` on the final release of a bare Windows-key tap. `TopBar.svelte` opens centered search through the existing centered path. Rust passes Windows-key down and chorded release paths through to preserve native Windows chords, suppresses bare final release to avoid Start activation, and suppresses stray/duplicate Windows-key releases when hook state is available or fails closed.
...
- `tests/searchClearButtons.test.mjs` covers top-bar and centered clear-button source wiring plus `clearLatestSearchQuery(...)` stale-response invalidation. `tests/windowsKeyOverride.test.mjs` covers the native Windows-key event name, top-bar-only target, TopBar listener path, required hook-install startup failure, fail-closed source contract, and main.rs install/uninstall lifecycle wiring; Rust tests in `windows_key_hook.rs` cover bare-tap suppression/open, Win+shortcut non-Windows-key pass-through, duplicate/stray release suppression, left/right overlap handling, unavailable hook-state fail-closed behavior, lifecycle idempotence, and top-bar target labeling.
...
- Bare Windows-key taps open centered search through an app-scoped native hook. Windows-key down events pass through so native Windows chords can preserve the physical modifier; bare final Windows-key release is suppressed and opens centered search, while chorded final release passes through. Stray/duplicate Windows-key releases still suppress, and unavailable hook state fails closed for Windows-key events.
...
- `tests/searchClearButtons.test.mjs` and `tests/windowsKeyOverride.test.mjs` cover clear-button source wiring, stale-response invalidation, native Windows-key open event routing, required hook startup behavior, fail-closed source contract, chord-preservation source contract, and hook lifecycle wiring. Rust `windows_key_hook.rs` tests cover tap/chord classification, pass-through Windows-key down for native chords, bare-release suppression/open, left/right overlap, unavailable hook-state fail-closed behavior, and lifecycle idempotence.

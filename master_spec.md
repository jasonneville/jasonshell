...
- Search P4 smoke focus: after static validation, live Windows smoke should verify top-bar and centered clear buttons keep keyboard focus, `Ctrl+Space` opens centered search without typing a stray space, Windows-key taps and Win+R/Win+D pass through to Windows, and app exit restores normal keyboard behavior.
...
- Startup sequence:
  ...
  5. On Windows, call `windows_key_hook::install_windows_key_hook(app.handle().clone())` to bind `Ctrl+Space` to centered search while leaving Windows-key shell behavior and other Ctrl chords to the OS. This hook is required; install failure returns a setup error instead of starting JasonShell in a degraded mode.
...
- Search data flow:
  ...
  - `SEARCH_HOTKEY_OPEN_SEARCH_EVENT = 'search:open-centered'` is listened by `TopBar.svelte`; native `Ctrl+Space` hook events call `openCenteredPanel({ publishCurrentPayload: true })` and refocus the top-bar search input after `tick()`.
- Pin data flow:
...
- `src-tauri/src/windows_key_hook.rs` owns the Windows-only low-level keyboard hook, `Ctrl+Space` classifier, idempotent install/uninstall, and `search:open-centered` event emission to `crate::shell_windows::TOP_BAR_LABEL`; it must never target `search-panel` directly, capture Windows-key behavior, or log/persist key data.
...
- `search:open-centered`: emitted by `windows_key_hook.rs` to `top-bar` on `Ctrl+Space` Space keydown while either Ctrl key is held. `TopBar.svelte` opens centered search through the existing centered path. Rust suppresses only the Space keydown/up belonging to the hotkey chord, suppresses repeat Space keydown while the chord is held, passes other Ctrl chords through, and lets Windows-key behavior pass through to the OS.
...
- `tests/searchClearButtons.test.mjs` covers top-bar and centered clear-button source wiring plus `clearLatestSearchQuery(...)` stale-response invalidation. `tests/windowsKeyOverride.test.mjs` covers the native search-hotkey event name, top-bar-only target, TopBar listener path, required hook-install startup failure, unavailable-state pass-through source contract, no Windows-key capture, and main.rs install/uninstall lifecycle wiring; Rust tests in `windows_key_hook.rs` cover `Ctrl+Space` open/suppression, repeat suppression, other chord pass-through, unavailable-state pass-through, lifecycle idempotence, and top-bar target labeling.
...
- `Ctrl+Space` opens centered search through an app-scoped native hook. Space keydown while either Ctrl key is held emits `search:open-centered` to `top-bar` and suppresses that Space keydown/up so no stray space is typed. Repeated Space keydowns while the chord is held are suppressed without reopening. Other Ctrl chords and all Windows-key behavior pass through to the OS; unavailable hook state also passes through.
...
- `tests/searchClearButtons.test.mjs` and `tests/windowsKeyOverride.test.mjs` cover clear-button source wiring, stale-response invalidation, native `Ctrl+Space` open event routing, required hook startup behavior, unavailable-state pass-through source contract, no Windows-key capture, and hook lifecycle wiring. Rust `windows_key_hook.rs` tests cover `Ctrl+Space` classification, Space suppression/open, repeated Space suppression, other Ctrl chord pass-through, unavailable hook-state pass-through behavior, and lifecycle idempotence.

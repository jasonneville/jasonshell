...
- Stack Browser terminal launch/streaming trust, 2026-05-09: Phases 2-4 of `terminal_plan.md` make the CLI view show compact startup, waiting, exited, or failed state instead of an unexplained blank black terminal before the first PTY byte, make output/geometry push-first under normal load, and move xterm ownership out of the large Stack Browser surface. `StackPopupSurface.svelte` now keeps Files/CLI view mode, terminal profile loading, cwd-to-folder synchronization, and close/error integration; `StackTerminalPane.svelte` owns the xterm instance, Fit/WebGL/WebLinks/Search/Serialize/Unicode 11 addons, persistent event listener cleanup, output queue, resize dispatch, search UI, and terminal actions. The backend reader emits live `stack-terminal:output` chunks to the session target, while polling is a 1500ms watchdog/recovery fallback. Frontend event and poll chunks are queued per active session, stale session ids are dropped, sequence order is preserved before xterm writes, and chunks flush through `requestAnimationFrame`; retained terminal replay output is bounded to 256 KiB for xterm reattachment/clear replay while xterm owns visual scrollback. xterm fitting waits for Svelte paint plus `requestAnimationFrame`, coalesces resize observer fits, dedupes same-size resize IPC, calls `FitAddon.fit()`, then sends rows/cols and pixel dimensions through `resizeStackTerminal(...)` / `resize_stack_terminal`; `src-tauri/src/stack_popup/terminal.rs` validates resize bounds, calls the ConPTY master resize path, and stores latest rows/cols/pixel dimensions in session snapshots. Phase 4 adds `src/features/stack-browser/terminalViewModel.ts` link detection for safe URL, localhost, Windows path, relative path, and git-hash targets; WebGL is optional and falls back by disposing the addon on context loss. PowerShell startup no longer clears the first visible prompt/banner with a final `Clear-Host`, and the `cmd.exe /K` launch path derives a short trusted PowerShell executable path to avoid both quoted-full-path literal failure and arbitrary PATH-only launch. Source and Rust coverage lives in `tests/stackBrowserTerminal.test.mjs`, `tests/settingsPanelWiring.test.mjs`, and `cargo test --manifest-path src-tauri/Cargo.toml terminal`.
...
- Shell popup layout scroll P5 Phase 3/4, 2026-05-06: `TrayPanelSurface.svelte` keeps loading, error, and empty states outside the icon list scroller; large icon sets render inside `.tray-content`, which is the flex child with `min-height: 0` and `overflow: auto` while `.tray-panel` remains fixed-height with `overflow: hidden`. `SearchPanelSurface.css` uses a flex column root with a fixed header/input row, `.result-list { flex: 1 1 auto; min-height: 0; overflow-y: auto; }`, and no approximate `max-height: calc(...)` result sizing; `.empty-state` uses the same remaining-height lane. `ProcessManagerSurface.svelte` wraps the header and body in `.process-table-scroll` / `.process-table-content`; `.process-table-scroll` owns horizontal and vertical overflow, `.process-row-head` stays sticky inside that scroller, `.process-table-body` no longer owns independent horizontal overflow, and `.process-table-content { min-width: 48rem; }` keeps right-side status/action columns reachable by horizontal scroll at default 720px width. Coverage lives in `tests/shellPopupLayoutScrollPhase1.test.mjs`, `tests/trayPanelWiring.test.mjs`, `tests/searchPanelState.test.mjs`, and process-manager state/wiring tests.
...
- Stack Browser terminal renderer, 2026-05-09: `src/components/StackTerminalPane.svelte` renders embedded CLI output/input through `@xterm/xterm` and owns xterm lifecycle separate from `StackPopupSurface.svelte`. Addons are loaded deliberately: Fit for geometry, WebGL with context-loss fallback to default renderer, WebLinks plus a custom safe link provider, Search for Ctrl+F scrollback search, Serialize for explicit visible-terminal copy only, and Unicode 11 activated through `terminal.unicode.activeVersion = '11'`. xterm owns cursor rendering, key translation, selectable terminal text, and Ctrl+C copies xterm selection through `navigator.clipboard.writeText(...)` before normal terminal interrupt behavior continues when no selection exists. The xterm font uses `"Cascadia Mono", "Cascadia Code", Consolas, ui-monospace, SFMono-Regular, monospace` at 12.8px. When the terminal pane unmounts or the host div is recreated, the xterm instance and addons are disposed and later rehydrated from bounded replay output; this prevents black-screen reattachment to a detached DOM node. Clear-screen escape output compacts the retained terminal buffer and resets xterm before replay so old text does not remain underneath `clear`, `Clear-Host`, or full-screen/TUI redraws. Backend sessions still use `portable-pty` ConPTY through `src-tauri/src/stack_popup/terminal.rs`; PowerShell startup installs Unix-friendly aliases including `ls`, `ll`, `clear`, `cat`, `grep`, and `which`. PowerShell is launched under ConPTY through trusted `cmd.exe /K pwsh.exe ...`; the trusted PowerShell 7 directory is prepended to the child PATH and the command line intentionally avoids quoted full paths because `portable-pty`/cmd can pass quotes as literal `\"...\"`, causing startup errors such as `'\"C:\Program Files\PowerShell\7\pwsh.exe\"' is not recognized`.

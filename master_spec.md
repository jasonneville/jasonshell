...
- Stack Browser terminal launch trust, 2026-05-09: Phase 2 of `terminal_plan.md` makes the CLI view show compact startup, waiting, exited, or failed state instead of an unexplained blank black terminal before the first PTY byte. `StackPopupSurface.svelte` tracks terminal lifecycle, profile label, cwd, first-output state, startup timeout, serialized read/write/resize terminal IPC, and persistent-surface listener cleanup for `stack-terminal:output` / `stack-terminal:closed`; the backend reader emits live `stack-terminal:output` chunks, while polling remains a first-output/watchdog fallback and no longer emits duplicate output events. xterm fitting waits for Svelte paint plus `requestAnimationFrame`, coalesces resize observer fits, dedupes same-size resize IPC, calls `FitAddon.fit()`, then sends rows/cols and pixel dimensions through `resizeStackTerminal(...)` / `resize_stack_terminal`; `src-tauri/src/stack_popup/terminal.rs` validates resize bounds and calls the ConPTY master resize path. PowerShell startup no longer clears the first visible prompt/banner with a final `Clear-Host`, and the `cmd.exe /K` launch path derives a short trusted PowerShell executable path to avoid both quoted-full-path literal failure and arbitrary PATH-only launch. Source and Rust coverage lives in `tests/stackBrowserTerminal.test.mjs`, `tests/settingsPanelWiring.test.mjs`, and `cargo test --manifest-path src-tauri/Cargo.toml terminal`.
...
- Stack Browser terminal renderer, 2026-05-08: `src/components/StackPopupSurface.svelte` renders embedded CLI output/input through `@xterm/xterm` plus `@xterm/addon-fit`, not a custom textarea/input/pre prompt. xterm owns cursor rendering, key translation, selectable terminal text, and Ctrl+C copies xterm selection through `navigator.clipboard.writeText(...)` before normal terminal interrupt behavior continues when no selection exists. The xterm font uses `var(--js-font-sans)` to match Settings/JSON shell controls. When the view switches away from CLI or the terminal host div is recreated, the xterm instance is disposed and later rehydrated from `stackTerminalOutput`; this prevents black-screen reattachment to a detached DOM node. Clear-screen escape output compacts the retained terminal buffer and resets xterm before replay so old text does not remain underneath `clear`, `Clear-Host`, or full-screen/TUI redraws. Backend sessions still use `portable-pty` ConPTY through `src-tauri/src/stack_popup/terminal.rs`; PowerShell startup installs Unix-friendly aliases including `ls`, `ll`, `clear`, `cat`, `grep`, and `which`. PowerShell is launched under ConPTY through trusted `cmd.exe /K pwsh.exe ...`; the trusted PowerShell 7 directory is prepended to the child PATH and the command line intentionally avoids quoted full paths because `portable-pty`/cmd can pass quotes as literal `\"...\"`, causing startup errors such as `'\"C:\Program Files\PowerShell\7\pwsh.exe\"' is not recognized`.

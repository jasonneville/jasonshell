...
- Primary surfaces: `top-bar`, `bottom-bar`, `task-preview`, `search-panel`, `stack-popup`, `process-manager`, `control-plane`, `settings-panel`, `tray-panel`, `command-panel`, and `audio-panel`, all routed through a shared `index.html` and selected by Tauri webview window label.
...
- Search accessibility/control P4, 2026-05-03: `TopBar.svelte` and centered `SearchPanelSurface.svelte` now render keyboard-accessible clear buttons only when their visible query/draft is non-empty. Clear actions route through the same immediate query reset path (`publishImmediateSearchInputState('')`, `startImmediateSearchQueryExecution(request)`, and centered `queueQueryEmit('')`) so latest-query sequence gates reject stale provider responses; they do not call providers, native show APIs, or panel publish paths directly. `src/features/search/searchUxState.ts` exports `clearLatestSearchQuery(...)` for explicit stale-response invalidation. Windows-only `src-tauri/src/windows_key_hook.rs` installs an app-lifecycle-scoped low-level keyboard hook after startup warm begins: bare Left/Right Windows-key taps are suppressed from the default Start menu and emit `search:open-centered` to `top-bar`, while Win+shortcuts such as Win+R and Win+D pass through. The hook stores only tap/chord state, performs no keystroke logging or persistence, and is uninstalled during app exit before AppBar cleanup so default OS handling is restored.
...
- Search overhaul Phase 6 UI hot path, 2026-04-29: `TopBar.svelte` now routes visible typed search through `src/lib/searchEngine.ts` and the Rust `search_engine` command instead of `searchCatalog.ts`, `searchRanking.ts`, or `systemSearch.ts`. `handleSearchInput` only forwards the input value; `applySearchQuery` synchronously updates `searchQuery`, publishes a `typing` pending/current-best `SearchPanelPayload`, opens the configured panel idempotently, and schedules provider work with a zero-delay latest-only request sequence. `createLatestSearchQueryController` in `src/features/search/searchUxState.ts` rejects stale Rust responses so older provider results cannot overwrite newer visible state, while provider calls are not serialized behind stale in-flight work. The panel result adapter preserves `actionId`, `actionArgs`, path/url/copy fields, provider IDs including `settings` and `local`, and `runControlPanel` activation uses the new Rust `run_control_panel` command with validated applet arguments instead of treating arguments as a shell path. `search/providers/open_windows.rs` ranks frontend-supplied open-window context rows so removing the old TypeScript catalog does not drop window search. `SearchPanelSurface.svelte` remains render-only.
...
- System tray backend + tray-panel UI, 2026-05-01 Phase 2: `src-tauri/src/system_tray.rs` remains the shipped Windows relay backend (`list_system_tray_icons`, `invoke_system_tray_icon`) with Explorer visible/overflow/secondary-taskbar discovery, source-qualified snapshot IDs (`tray-notify:*`, `overflow:*`), real Explorer label extraction, and native icon extraction fallback behavior. Phase 2 now ships `src-tauri/src/tray_panel.rs`, `src/components/TrayPanelSurface.svelte`, and top-bar tray-button wiring so the hidden-icons popup is a dedicated anchored webview (`tray-panel`) that closes on focus loss and emits `tray-panel:closed` to keep top-bar popup state synchronized.
- Command panel + quick-command editor UX, 2026-05-01 Phase 4: `src-tauri/src/command_panel.rs` now ships anchored command-panel show/hide commands (`show_command_panel`, `hide_command_panel`) with top-bar closed-event sync (`command-panel:closed`), while `src/components/CommandPanelSurface.svelte` and `src/components/CommandPanelSurface.css` ship the popup editor/list UX for global `quickCommands.entries` including run/edit/delete actions, one-arg-per-line textarea conversion, inline validation, full settings persistence, and run-on-success close behavior. `TopBar.svelte` now includes a terminal-style `>_` command button immediately left of tray with popup exclusivity across search/tray/audio/command.
...
- Search P4 smoke focus: after static validation, live Windows smoke should verify top-bar and centered clear buttons keep keyboard focus, bare Windows-key tap opens centered search without launching Start, Win+R/Win+D still pass through to Windows, and app exit restores normal Windows-key behavior.
...
- Startup sequence:
  ...
  5. On Windows, call `windows_key_hook::install_windows_key_hook(app.handle().clone())` to bind bare Windows-key taps to centered search while leaving Win+chords to the OS.
  6. On Windows, call `appbar::activate_shell_surfaces` to register, position, show, and stabilize top/bottom appbars.
  7. On non-Windows, show top/bottom windows without native AppBar integration and use a no-op Windows-key hook shim.
- Shutdown/close behavior:
  ...
  - App exit uninstalls the Windows-key hook, then attempts appbar cleanup.
...
- `on_window_event` has special cases: `stack-popup` hides on `WindowEvent::Focused(false)` unless `StackPopupRuntimeState` is holding focus loss during an internal delete command, `search-panel` emits `search-panel:closed` then hides on `WindowEvent::Focused(false)`, `process-manager` emits `process-manager:closed` then hides on `WindowEvent::Focused(false)`, `settings-panel` hides on `WindowEvent::Focused(false)`, `tray-panel` emits `tray-panel:closed` then hides on `WindowEvent::Focused(false)`, `command-panel` emits `command-panel:closed` then hides on `WindowEvent::Focused(false)`, `audio-panel` emits `audio-panel:closed` then hides on `WindowEvent::Focused(false)`, and primary shell surfaces (`top-bar`, `bottom-bar`) clean AppBars and exit on close/destroy. `control-plane` currently has explicit show/hide commands and no focus-loss lifecycle event. Avoid adding generic close handlers that would make auxiliary windows terminate the shell.
...
- `app.run()` repeats Windows-key hook uninstall and AppBar cleanup on `RunEvent::Exit` and `ExitRequested`. Cleanup is intentionally idempotent through `ShellRuntimeState.cleaned_up`; hook uninstall is also idempotent.
...
| `command-panel` | `src/components/CommandPanelSurface.svelte` + `CommandPanelSurface.css` | `command_panel.rs`, `quick_commands.rs`, `settings.rs`, `shell_windows.rs` | Top-bar anchored quick-command editor and launcher for global `quickCommands.entries`, with run/edit/delete, one-arg-per-line args, inline validation, and run-on-success close behavior. |
...
- Search data flow:
  - `applySearchQuery()` synchronously updates the top-bar query, queues a pending/current-best `SearchPanelPayload`, opens the configured panel idempotently, and schedules provider work.
  - `scheduleSearchEngine()` starts zero-delay latest-only work against `src/lib/searchEngine.ts`; `loadSearchEngineResults()` calls Rust `search_engine` with open-window context and ignores stale query/sequence responses.
  ...
  - `WINDOWS_KEY_OPEN_SEARCH_EVENT = 'search:open-centered'` is listened by `TopBar.svelte`; native hook events call `openCenteredPanel({ publishCurrentPayload: true })` and refocus the top-bar search input after `tick()`.
- Pin data flow:
...
- `src-tauri/src/windows_key_hook.rs` owns the Windows-only low-level keyboard hook, tap/chord classifier, idempotent install/uninstall, and `search:open-centered` event emission to `crate::shell_windows::TOP_BAR_LABEL`; it must never target `search-panel` directly or log/persist key data.
...
- `audio-panel:closed`: emitted by Rust `main.rs` focus-loss handling and `hide_audio_panel`; `TopBar.svelte` listens to clear `audioOpen`/`aria-expanded` state.
- `search-index:refreshed`: emitted by backend search index warmer, listened by top bar to refresh active indexed search.
- `search:open-centered`: emitted by `windows_key_hook.rs` to `top-bar` on a bare Windows-key tap; `TopBar.svelte` opens centered search through the existing centered path and preserves Win+shortcut OS behavior by ignoring chorded key sequences in Rust.
...
- `tests/searchClearButtons.test.mjs` covers top-bar and centered clear-button source wiring plus `clearLatestSearchQuery(...)` stale-response invalidation. `tests/windowsKeyOverride.test.mjs` covers the native Windows-key event name, top-bar-only target, TopBar listener path, and main.rs install/uninstall lifecycle wiring; Rust tests in `windows_key_hook.rs` cover bare-tap suppression/open, Win+shortcut pass-through, repeat handling, lifecycle idempotence, and top-bar target labeling.
...
- Bare Windows-key taps open centered search through an app-scoped native hook; Windows-key chords remain OS-owned shortcuts.
...
- Panel payload ownership:
  ...
  - TopBar does not synchronously rank results in the input handler. It publishes a cheap current-query payload, then schedules `searchEngine` on a zero-delay latest-only request gate from `searchUxState.ts`; only the latest queued query may apply and publish results.
...
- `tests/searchClearButtons.test.mjs` and `tests/windowsKeyOverride.test.mjs` cover clear-button source wiring, stale-response invalidation, native Windows-key open event routing, and hook lifecycle wiring. Rust `windows_key_hook.rs` tests cover tap/chord classification and lifecycle idempotence.
...
- `src/lib/quickCommands.ts`: quick-command settings normalization, one-arg-per-line textarea conversion helpers, settings read/write projection helpers, and typed `run_quick_command` wrapper.
...
- Tauri capability files are split per current surface under `src-tauri/capabilities/*.json`: `top-bar` remains in `default.json`; `bottom-bar`, `task-preview`, `search-panel`, `stack-popup`, `process-manager`, `control-plane`, `settings-panel`, `tray-panel`, and `command-panel` each have a single-window capability file.
...
- `jasonshell-settings-v1.json`:
  ...
  - `quickCommands.entries` stores `QuickCommandEntry { id, label, mode, targetPath, args, cwd }`. Validation requires slug-safe unique ids, trimmed non-empty labels, direct-mode target as absolute path or safe command token, script-mode absolute target paths, optional absolute cwd, argument control-character rejection, and secret-like argument rejection before persistence and execution.
...
- 2026-05-03 `[USER]` REQUESTED/IN_PROGRESS: Implement `search-accessibility-and-control-P4.md` with strict TDD phase gates. Required outcomes: top-bar and centered search clear buttons, stale search response rejection on clear, app-scoped bare Windows-key override that opens centered search, Win+shortcuts pass through, no keystroke logging, lifecycle cleanup, focused RED/GREEN evidence, spec update, and final validation.
- 2026-05-03 `[CODE]` COMPLETED: Added `tests/searchClearButtons.test.mjs` and `tests/windowsKeyOverride.test.mjs`; exported `clearLatestSearchQuery(...)` from `src/features/search/searchUxState.ts`; added accessible `MeltActionButton` clear controls and styling in `TopBar.svelte`/`TopBar.css` and `SearchPanelSurface.svelte`/`.css`; added `src-tauri/src/windows_key_hook.rs` with Windows-key tap/chord classifier, idempotent native hook install/uninstall, top-bar-only `search:open-centered` event emission, non-Windows shim, and main.rs startup/exit lifecycle wiring; updated `TopBar.svelte` to listen for the event and open centered search through the existing path.
- 2026-05-03 `[TOOL]` VALIDATED: Phase 1 RED confirmed by missing `clearLatestSearchQuery` export; GREEN clear-button suite passed via `npm run test:node -- searchClearButtons` (407 total, 404 pass, 0 fail, 3 todo). Phase 3 RED confirmed by `windows_key_hook` Rust tests panicking before classifier/lifecycle implementation; GREEN passed with `npm run cargo:test` (336 passed, 0 failed, 1 ignored) and `npm run test:node -- windowsKeyOverride` (410 total, 407 pass, 0 fail, 3 todo). Final `npm run validate` passed after spec update and warning cleanup: Node 410 total/407 pass/3 todo; Rust 336 passed/1 ignored; Cargo check clean.

- 2026-05-03 `[USER]` REQUESTED/IN_PROGRESS: Correct Stack Browser archive and Properties behavior with TDD gates. Required outcomes: ZIP context menu exposes built-in extraction separately from 7-Zip extraction; RAR uses 7-Zip only; extraction actually waits and refreshes so new files/folders appear; double-clicking ZIP rows browses archive contents inside Stack Browser; Properties opens the real Windows item Properties dialog, not Explorer.
- 2026-05-03 `[CODE]` COMPLETED: Updated Stack Browser action planning, UI wiring, and backend commands for explicit archive extractors and Properties. Added `StackArchiveExtractor` through TS/Rust IPC, four ZIP extraction menu choices, RAR-only 7-Zip choices, blocking extraction status before refresh, ZIP virtual browsing via `read_stack_folder`/`zip` crate, double-click ZIP navigation, and Windows Shell Properties verb invocation through PowerShell COM. Updated command constants/contracts/registration and tests in `tests/stackBrowserContextActions.test.mjs` plus Rust stack-popup tests.
- 2026-05-03 `[TOOL]` VALIDATED: RED confirmed before implementation by failing context-action, archive-plan, Properties-plan, and ZIP-browsing tests. GREEN focused checks passed: `npm run test:node -- stackBrowserContextActions` (402 total, 399 pass, 0 fail, 3 todo); `cargo test --manifest-path src-tauri/Cargo.toml read_stack_folder_page_lists_zip_contents_as_stack_rows`; `cargo test --manifest-path src-tauri/Cargo.toml archive` (4 passed); `cargo test --manifest-path src-tauri/Cargo.toml stack_item_properties_plan_validates_existing_paths`; and `npm run check` (0 errors, 0 warnings).

- 2026-05-03 `[USER]` REQUESTED/IN_PROGRESS: Complete `multi_fix_plan.md` phases 5 and 6 with TDD gates before advancement. Required outcomes: Settings Panel Power section for Sleep, Restart, Turn Off; in-panel confirmation only; enum-only TS/Rust command boundary; safe Windows power execution; inline failure display; `master_spec.md` functional/ledger updates; focused suites after each phase; and final `npm run validate` attempt.
- 2026-05-03 `[CODE]` COMPLETED: Multi-fix Phase 5/6 implemented. Added `src-tauri/src/system_power.rs` with serde enum validation, `trigger_system_power_action`, Windows sleep API path, and restart/shutdown `shutdown.exe` argument-vector plans. Registered `system_power` in `src-tauri/src/main.rs`, added `Win32_System_Power` feature in `src-tauri/Cargo.toml`, added `triggerSystemPowerAction` IPC constant and enum-only wrapper in `src/lib/settingsPanel.ts`, and added Settings Panel Power UI/confirmation/error styling in `src/components/SettingsPanelSurface.svelte` and CSS. Added `tests/settingsPowerActionsPhase5.test.mjs` and `tests/multiFixPhase6SpecValidation.test.mjs`. Updated Settings panel spec, residual live-smoke scope, and validation notes for search typing, Stack Browser rectangle selection, bottom quick icons, VS Code folder actions, settings confirmations, and `npm run validate`.
- 2026-05-03 `[TOOL]` VALIDATED: Phase 5 RED confirmed by missing `system_power.rs`; GREEN focused checks passed with `cargo test --manifest-path src-tauri/Cargo.toml system_power` (2 passed) and `npm run check` (0 errors, 0 warnings). Phase 6 RED confirmed by `tests/multiFixPhase6SpecValidation.test.mjs` failing before spec update. `npm run test:node` currently still stops on a pre-existing unrelated `tests/frontendUiPolicy.test.mjs` Stack Browser source-regex expectation for `listStackFolder(folderPath)` followed by `commitValidatedStackFolderListing(...)`; new phase 5 source tests pass before that blocker.
- 2026-05-02 `[USER]` REQUESTED/IN_PROGRESS: Follow-up implementation-worker fix for typed-search one-letter-behind adversarial findings. Expected surfaces: `src/components/TopBar.svelte`, `tests/searchTypingFreezePhase1.test.mjs`, `src/lib/searchPanelState.ts`, `tests/searchPanelState.test.mjs`, and `master_spec.md`. Required fixes: app-index refresh must not schedule old applied `searchQuery` while `searchInputDraft` differs during the 16ms deferred-apply window, and stray `FALSE1`/`FALSE2`/`here` debug logs must be removed and source-guarded.
- 2026-05-02 `[CODE]` COMPLETED: Updated `src/components/TopBar.svelte` app-index refresh handling to compute `refreshQuery` from the current draft when it differs from the applied query. During the deferred-apply window, refresh now resets provider-cache retry state and re-enters `queueSearchQueryProcessing(refreshQuery)` instead of calling `scheduleSearchEngine(searchQuery)` with the stale applied value. Removed stray `console.log("FALSE1")`, `console.log("FALSE2")`, and `console.log("here")` debug output from `src/lib/searchPanelState.ts`. Added source-level regression guards in `tests/searchTypingFreezePhase1.test.mjs` and `tests/searchPanelState.test.mjs`.
- 2026-05-02 `[TOOL]` VALIDATED: RED confirmed with `node --test tests\searchTypingFreezePhase1.test.mjs tests\searchPanelState.test.mjs` failing the new refresh-draft and debug-literal guards before implementation. GREEN validation passed with `npx tsc -p tsconfig.test.json`, `node --test tests\searchTypingFreezePhase1.test.mjs tests\searchPanelState.test.mjs` (33 passed), and `npm run check` (0 errors, 0 warnings).
- 2026-05-02 `[TOOL]` VALIDATED: Orchestrator final focused sweep passed with `npx tsc -p tsconfig.test.json`, `node --test tests\searchTypingFreezePhase1.test.mjs tests\searchPanelState.test.mjs tests\searchUxState.test.mjs tests\searchEngineContracts.test.mjs tests\searchAppCacheRefresh.test.mjs tests\centeredSearchSurface.test.mjs tests\searchOverhaulPhase6.test.mjs` (85 passed), `npm run check` (0 errors, 0 warnings), and scoped `git diff --check` on touched search files. `npm run build` still fails outside this search fix because existing dirty `vite.config.ts` / package changes add `vite-plugin-terminal`, causing Vite to fail resolving `/@id/__x00__virtual:terminal/console` from `index.html`.
- 2026-05-02 `[USER]` REQUESTED/IN_PROGRESS: Re-investigate persistent app-search trailing-Space workaround after same-query freshness retry did not fix live Spotify/Firefox/Brave results. Expected surfaces: app-provider cache timing/state handling in `TopBar.svelte` and `searchUxState.ts`, search refresh regression tests, and Rust app provider only if frontend cache-readiness evidence is insufficient. Required workflow: identify root cause, add RED test before implementation, implement root fix, and validate focused search gates plus standard checks.
- 2026-05-02 `[CODE]` COMPLETED: Root cause traced to app-provider cache readiness, not query text matching. `search_apps(...)` returns `cache=miss` or `cache=indexing` with zero app rows while `warm_app_index_async()` is still building the cache; pressing trailing Space submitted a second equivalent trimmed query after the cache had warmed. Added `tests/searchAppCacheRefresh.test.mjs` before implementation, then exported `shouldRetrySearchAfterProviderCacheWarm(...)` from `src/features/search/searchUxState.ts` and wired `src/components/TopBar.svelte` to schedule bounded, sequence-gated same-query retries from both progress and complete `providerTimings`.
- 2026-05-02 `[TOOL]` VALIDATED: RED confirmed with `node --test tests\searchAppCacheRefresh.test.mjs` failing on missing `shouldRetrySearchAfterProviderCacheWarm` before implementation. GREEN validation passed: `npx tsc -p tsconfig.test.json`; `node --test tests\searchAppCacheRefresh.test.mjs`; focused search sweep `node --test tests\searchAppCacheRefresh.test.mjs tests\searchTypingFreezePhase1.test.mjs tests\centeredSearchSurface.test.mjs tests\searchPanelState.test.mjs tests\searchUxState.test.mjs tests\searchOverhaulPhase0.test.mjs tests\searchOverhaulPhase6.test.mjs` (77 passed, 0 failed, 3 historical TODO); `cargo test --manifest-path src-tauri\Cargo.toml search::` (76 passed); `npm run check`; and `npm run build`. Full `npm run validate` passed check/build and the new search node tests, then stopped at the pre-existing unrelated `tests/frontendUiPolicy.test.mjs` Stack Browser source-regex expectation for `listStackFolder(folderPath)` followed by `commitValidatedStackFolderListing(...)`.
- 2026-05-02 `[USER]` REQUESTED/IN_PROGRESS: Begin implementation from repo-root `multi_fix_plan.md`, phase 1 only. Required outcomes: read and follow full multi-fix plan context, use TDD-first RED/GREEN flow for P0 search typing freeze, split top-bar input echo from search execution so rapid `Firefox` typing/paste updates visible input immediately, keep search provider/panel publish/grouping work behind latest-only deferred boundaries, preserve centered-search optimistic draft/keyboard/Escape/stale-response behavior, run all phase-1 focused tests plus `cargo test --manifest-path src-tauri/Cargo.toml search::`, and do not advance to phase 2 until phase-1 acceptance passes.
- 2026-05-02 `[USER]` REQUESTED/IN_PROGRESS: Fix follow-up search freshness bug where typing a query often does not populate the correct results until pressing Space. Expected surfaces: centered `SearchPanelSurface.svelte` query emit/payload handling, `TopBar.svelte` query processing/publish path, search freshness tests, and Rust search only if investigation proves backend query semantics are involved. Required workflow: investigate root cause, write RED regression tests first, implement, validate focused search gates.
- 2026-05-02 `[CODE]` COMPLETED: Added automatic same-query freshness retry for the search Space workaround. `tests/searchTypingFreezePhase1.test.mjs` now covers the RED regression first via `shouldRetrySearchFreshness(...)` and `TopBar.svelte` wiring expectations. `src/features/search/searchUxState.ts` exports the retry gate, and `src/components/TopBar.svelte` schedules/cancels a one-shot `searchFreshnessRetryTimer` that reruns the latest normalized query once when result freshness stalls, while keeping direct input echo and heavy search work deferred.
- 2026-05-02 `[TOOL]` VALIDATED: RED confirmed before implementation because `shouldRetrySearchFreshness` was missing. Final focused validation passed: `npx tsc -p tsconfig.test.json`; `node --test tests\searchTypingFreezePhase1.test.mjs tests\centeredSearchSurface.test.mjs tests\searchPanelState.test.mjs tests\searchUxState.test.mjs tests\searchOverhaulPhase0.test.mjs tests\searchOverhaulPhase6.test.mjs` (74 passed, 0 failed, 3 historical TODO); `cargo test --manifest-path src-tauri\Cargo.toml search::` (76 passed); `npm run check`; and `npm run build`. Full `npm run validate` passed check/build and all search freshness tests, then stopped at the known unrelated `tests/frontendUiPolicy.test.mjs` Stack Browser source-regex expectation.
- 2026-05-02 `[CODE]` COMPLETED: Closed `multi_fix_plan.md` phase 1 / P0 search typing freeze with TDD-first implementation. Added `tests/searchTypingFreezePhase1.test.mjs` before implementation, then updated `src/components/TopBar.svelte` so direct top-bar input and centered-query events update `searchInputDraft` immediately and enqueue latest-only query execution behind `SEARCH_QUERY_PROCESSING_DELAY_MS = 16`; added `createLatestSearchExecutionQueue` in `src/features/search/searchUxState.ts`; hardened close handling to cancel queued/scheduled search work and invalidate stale responses; changed Rust `search_engine` in `src-tauri/src/search/mod.rs` to an async Tauri command using `tauri::async_runtime::spawn_blocking` for synchronous provider coordination.
- 2026-05-02 `[TOOL]` VALIDATED: Phase 1 RED tests failed before implementation for direct input deferral and async Rust boundary, then passed after code. Final focused gates passed: `npx tsc -p tsconfig.test.json`; `node --test tests\searchTypingFreezePhase1.test.mjs tests\centeredSearchSurface.test.mjs tests\searchPanelState.test.mjs tests\searchUxState.test.mjs tests\searchOverhaulPhase0.test.mjs tests\searchOverhaulPhase6.test.mjs` (73 passed, 0 failed, 3 historical TODO); `cargo test --manifest-path src-tauri\Cargo.toml search::` (76 passed); `npm run check` (0 errors, 0 warnings); `npm run build`; `npm run cargo:test` (307 passed, 0 failed, 1 ignored live-only); and `npm run cargo:check`. `npm run validate` passed check/build and all search typing-freeze tests but stopped in `tests/frontendUiPolicy.test.mjs` on an unrelated Stack Browser source-regex expectation for `listStackFolder(folderPath)` / `commitValidatedStackFolderListing(...)`.
- 2026-05-02 `[USER]` REQUESTED/IN_PROGRESS: Complete `stack_browser_plan.md` phase 5 after confirming phases 0-4 remain green. Required outcomes: TDD-first phase-5 diagnostics/regression/live-validation closure, including first-paint/metadata-complete/icon-queue-complete timing visibility, icon-cache hit/miss diagnostics, large-folder mixed regression coverage, and acceptance-criteria-aligned validation evidence without regressing prior phases.
- 2026-05-02 `[CODE]` COMPLETED: Closed `stack_browser_plan.md` phase 5 with TDD-first implementation and no phase-0..4 regressions. Added phase-tagged stack-browser diagnostics contract/wiring in `src/lib/stackPopup.ts` (`page`, `first-paint`, `metadata-complete`, `icon-queue-complete`) including first-paint/metadata/icon-queue timings and icon cache hit/miss/fallback counters; added icon-queue completion diagnostics aggregation in `src/components/StackPopupSurface.svelte`; extended Rust paging diagnostics payload fields in `src-tauri/src/stack_popup/models.rs`; and fixed paging session elapsed timing in `src-tauri/src/stack_popup/paging.rs` so continuation pages report true per-open elapsed folder timing.
- 2026-05-02 `[TOOL]` VALIDATED: Phase-5 RED/GREEN and regression gates passed. RED evidence: `node --test tests/stackPopupPagingPhase5Wiring.test.mjs tests/stackPopupState.test.mjs` (3 failing phase-5 diagnostics assertions) and `cargo test --manifest-path src-tauri/Cargo.toml continuation_diagnostics_keep_session_elapsed_time_for_metadata_completion_timing` (failed before session-elapsed timing fix). GREEN evidence: `node --test tests/stackPopupPagingPhase5Wiring.test.mjs tests/stackPopupState.test.mjs`; `cargo test --manifest-path src-tauri/Cargo.toml stack_popup::paging`; phase-0..5 Node regression sweep (`tests/stackPopupDiagnosticsWiring`, `stackPopupPagingPhase1..5Wiring`, `stackPopupState`, `stackPopupViewModel`); `cargo test --manifest-path src-tauri/Cargo.toml stack_popup`; `npm run check`; and `npm run build` (all pass).
- 2026-05-02 `[USER]` REQUESTED/IN_PROGRESS: Implement `stack_browser_plan.md` phases 3-4 with strict sequential TDD gates. Required outcomes: phase 3 lazy icon pipeline + stack-browser icon cache + bounded resolution concurrency + progressive icon updates; phase 4 progressive UX hardening for status semantics, virtualization-safe progressive append, and focus-thrash reduction while preserving stack-browser behaviors.
- 2026-05-02 `[CODE]` COMPLETED: Implemented phases 3-4. Phase 3 added backend stack icon batch/cache command path (`resolve_stack_item_icons`) with bounded batch helper and cache-hit metadata (`src-tauri/src/stack_popup/icons.rs`, command wiring in `stack_popup.rs`/`main.rs`/contracts/IPC), frontend wrapper/types (`src/lib/stackPopup.ts`), state-level in-place icon update merge (`applyStackEntryIconUpdates` in `src/lib/stackPopupState.ts`), and bounded lazy icon hydration queue in `StackPopupSurface.svelte` (batch=24, concurrency=2, reopen cache reuse, non-blocking fallback). Phase 4 added progressive UX hardening: distinct icon-progress status helper (`stackIconHydrationStatus`), virtualization-visible-window event emission (`stack-browser:rows-window-changed`), and guarded per-page focus helper (`maybeFocusDetailsGridAfterPageAppend`) to avoid unconditional per-page focus churn during progressive append.
- 2026-05-02 `[TOOL]` VALIDATED: Phase 3 RED/GREEN evidence: RED with `node --test tests/stackPopupPagingPhase3Wiring.test.mjs tests/stackPopupState.test.mjs` and `cargo test --manifest-path src-tauri/Cargo.toml stack_popup` failing on missing icon APIs, then GREEN with `npx tsc -p tsconfig.test.json`, `node --test tests/stackPopupPagingPhase3Wiring.test.mjs tests/stackPopupState.test.mjs`, and `cargo test --manifest-path src-tauri/Cargo.toml stack_popup` (pass). Phase 4 RED/GREEN evidence: RED with `node --test tests/stackPopupPagingPhase4Wiring.test.mjs tests/stackPopupState.test.mjs` (missing rows-window/focus helper/icon-status exports), then GREEN with `npx tsc -p tsconfig.test.json` and `node --test tests/stackPopupPagingPhase4Wiring.test.mjs tests/stackPopupState.test.mjs tests/stackPopupViewModel.test.mjs` (all pass).
- 2026-05-01 `[USER]` REQUESTED/IN_PROGRESS: Implement `stack_browser_plan.md` phases 0-2 sequentially with strict TDD gates per phase (tests/acceptance before moving to next phase). Expected surfaces: Stack Browser paging architecture (`src-tauri/src/stack_popup/paging.rs`, `src/lib/stackPopup.ts`, `src/components/StackPopupSurface.svelte`, state/tests) plus master-spec updates and focused validation evidence.
- 2026-05-01 `[CODE]` COMPLETED: Implemented `stack_browser_plan.md` phases 0-2 with TDD gating. Phase 0 added stack-browser paging diagnostics contracts/hooks in Rust + TypeScript and documented root-cause/fix order in the spec. Phase 1 changed stack-folder page materialization to metadata-only rows (`stack_item_metadata_from_path`) so paging no longer blocks on inline shell icon extraction. Phase 2 replaced offset-only fake paging with stable backend listing sessions (`sessionId`), stale-session rejection, snapshot-based continuation (no per-page re-enumeration), and steady paging constants (60 initial / 120 follow-up) in `src/lib/stackPopup.ts`.
- 2026-05-01 `[TOOL]` VALIDATED: Phase 0 RED/GREEN: `node --test tests/stackPopupDiagnosticsWiring.test.mjs tests/stackPopupState.test.mjs` plus `cargo test --manifest-path src-tauri/Cargo.toml stack_folder_page_diagnostics_defaults_to_zeroed_metrics`. Phase 1 RED/GREEN: `node --test tests/stackPopupPagingPhase1Wiring.test.mjs tests/stackPopupState.test.mjs`, `cargo test --manifest-path src-tauri/Cargo.toml deserializes_stack_item_without_icon_payload`, and `cargo test --manifest-path src-tauri/Cargo.toml reads_folder_details_with_folders_first`. Phase 2 RED/GREEN: `node --test tests/stackPopupPagingPhase2Wiring.test.mjs tests/stackPopupState.test.mjs`, `cargo test --manifest-path src-tauri/Cargo.toml stack_folder_session_paging_returns_stable_unique_rows_across_pages`, `stale_stack_folder_session_is_rejected_after_new_session_starts`, `session_continuation_uses_stable_snapshot_without_rereading_directory`, and updated `paginates_large_stack_folders_without_truncating_metadata`. Final focused sweep passed: `npx tsc -p tsconfig.test.json`, `node --test tests/stackPopupDiagnosticsWiring.test.mjs tests/stackPopupPagingPhase1Wiring.test.mjs tests/stackPopupPagingPhase2Wiring.test.mjs tests/stackPopupState.test.mjs`, and `cargo test --manifest-path src-tauri/Cargo.toml stack_popup`.
- 2026-05-01 `[USER]` REQUESTED/IN_PROGRESS: Complete `tray_and_commands_plan.md` phase 5 fully after phases 0-4, including full contract sync, expanded tray/command/quick-command validation coverage, regression verification across prior phases, and live Windows smoke checks where executable from this environment.
- 2026-05-01 `[CODE]` COMPLETED: Closed `tray_and_commands_plan.md` phase 5 and finalized release hardening. Updated `src-tauri/src/appbar.rs` so retry-exhausted work-area restore mismatches are treated as warning-only in startup/cleanup retry wrappers (preventing transient Explorer timing mismatches from crashing setup), and refreshed `docs/smoke-test-windows.md` to the current shipped phase-0..5 tray/command/audio smoke checklist.
- 2026-05-01 `[TOOL]` VALIDATED: Re-ran full regression/acceptance gates after phase-5 closure and appbar hardening: focused Rust (`cargo test --manifest-path src-tauri/Cargo.toml system_tray`, `tray_panel`, `command_panel`, `quick_commands`, `settings`, `appbar`), focused Node (`node --test tests/commandPanelWiring.test.mjs tests/trayPanelWiring.test.mjs tests/quickCommands.test.mjs tests/contractsSettings.test.mjs tests/systemTray.test.mjs`), `npm run cargo:check`, `npm run test:node`, and full `npm run validate` (all passing). Live startup smoke via `cargo run --manifest-path src-tauri/Cargo.toml --no-default-features --` held >25s and logged normal runtime shell metrics for top/bottom bars with no setup panic.
- 2026-05-01 `[USER]` REQUESTED/IN_PROGRESS: Move to `tray_and_commands_plan.md` phase 4 with strict TDD. Required outcomes: add `command-panel` surface + anchored popup backend, top-bar command button left of tray with popup exclusivity, quick-command editor/run UX, and full acceptance validation without regressions.
- 2026-05-01 `[CODE]` COMPLETED: Phase-4 command-panel slice shipped. Added backend `src-tauri/src/command_panel.rs` and registered `show_command_panel`/`hide_command_panel` with focus-loss `command-panel:closed` emission in `main.rs`; added `command-panel` shell window/capability/contract wiring across `shell_windows.rs`, `contracts.rs`, `src/ipc/{commands,events,surfaces}.ts`, `src/lib/shellSurface.ts`, and `src/App.svelte`; added frontend wrapper `src/lib/commandPanel.ts`; added `src/components/CommandPanelSurface.svelte` + CSS with saved-command Run/Edit/Delete list, one-arg-per-line args editor, inline validation, settings persistence, and run-on-success close; updated `TopBar.svelte`/`TopBar.css` with terminal-style `>_` command button immediately left of tray and mutual-exclusivity handling across search/tray/audio/command.
- 2026-05-01 `[TOOL]` VALIDATED: TDD RED confirmed by failing tests before implementation (`node --test tests/commandPanelWiring.test.mjs tests/contractsSettings.test.mjs` missing command-panel files). GREEN validation passed after implementation with focused Node tests (`node --test tests/commandPanelWiring.test.mjs tests/trayPanelWiring.test.mjs tests/quickCommands.test.mjs tests/contractsSettings.test.mjs tests/systemTray.test.mjs`), focused Rust tests (`cargo test --manifest-path src-tauri/Cargo.toml command_panel`, `cargo test --manifest-path src-tauri/Cargo.toml tray_panel`, `cargo test --manifest-path src-tauri/Cargo.toml quick_commands`, `cargo test --manifest-path src-tauri/Cargo.toml system_tray`), `npm run cargo:check`, `npm run test:node`, and full `npm run validate` (check/build/node/rust/cargo all passing).
- 2026-05-01 `[USER]` REQUESTED/IN_PROGRESS: Continue `tray_and_commands_plan.md` by validating phases 0-2 and phase-3 readiness gates, then fully implement phase 3 (global quick-command settings model + safe argv-only runner) and satisfy phase-3 acceptance behavior.
- 2026-05-01 `[CODE]` COMPLETED: Phase-3 quick-command slice shipped. Added global `quickCommands.entries` settings contract in `src-tauri/src/settings.rs` and `src/lib/settings.ts`; added strict quick-command validation (slug-safe unique ids, trimmed label, direct/script target constraints, optional absolute cwd, control-char and secret-like argument rejection); added backend `src-tauri/src/quick_commands.rs` and registered `run_quick_command` in `main.rs`/`contracts.rs`; added frontend `src/lib/quickCommands.ts` for normalization, one-arg-per-line args conversion, settings read/write projection, and constant-backed run wrapper; updated IPC command constants and contract tests.
- 2026-05-01 `[TOOL]` VALIDATED: Confirmed phase-3 readiness and completion gates. Pre-implementation readiness checks passed: `cargo test --manifest-path src-tauri/Cargo.toml tray_panel`, `cargo test --manifest-path src-tauri/Cargo.toml system_tray`, `node --test tests/trayPanelWiring.test.mjs tests/contractsSettings.test.mjs tests/systemTray.test.mjs`, `npm run cargo:check`, `npm run test:node`, and `npm run validate`. Post-implementation phase-3 checks passed: `cargo test --manifest-path src-tauri/Cargo.toml quick_commands`, `cargo test --manifest-path src-tauri/Cargo.toml settings`, `npx tsc -p tsconfig.test.json`, `node --test tests/quickCommands.test.mjs tests/contractsSettings.test.mjs`, and full `npm run validate` (check/build/node/rust/cargo all passing).
- 2026-05-01 `[USER]` REQUESTED/IN_PROGRESS: Implement `tray_and_commands_plan.md` phase 2 only with test-driven development. Required outcomes: add shipped `tray-panel` service/surface wiring and top-bar tray-button integration (anchored popup, focus-loss close, closed event sync, popup exclusivity with search/audio), complete phase-2 acceptance checks with tests written first and passing after implementation.
- 2026-05-01 `[CODE]` COMPLETED: Phase-2 tray-panel slice shipped. Added backend `tray_panel.rs` (`show_tray_panel`, `hide_tray_panel`, right-edge anchor/clamp helpers, top-bar closed-event sync), registered commands in `main.rs`, created `tray-panel` shell window/capability/contract wiring, added frontend tray wrappers (`src/lib/trayPanel.ts`), added `TrayPanelSurface.svelte` icon-grid popup with loading/error/empty states and left/right relay calls, and integrated a down-arrow tray button in `TopBar.svelte` immediately left of audio with mutual-exclusivity state handling across search/audio/tray.
- 2026-05-01 `[TOOL]` VALIDATED: TDD RED confirmed with `node --test tests/trayPanelWiring.test.mjs` failing before implementation (missing tray-panel files). GREEN validation passed with `node --test tests/trayPanelWiring.test.mjs`, `cargo test --manifest-path src-tauri/Cargo.toml tray_panel`, `cargo test --manifest-path src-tauri/Cargo.toml system_tray`, `node --test tests/trayPanelWiring.test.mjs tests/contractsSettings.test.mjs tests/systemTray.test.mjs`, `npm run cargo:check`, `npm run test:node`, and full `npm run validate` (check/build/node/rust/cargo all passing).
- 2026-05-01 `[USER]` REQUESTED/IN_PROGRESS: Execute `tray_and_commands_plan.md` phases 0 and 1 only. Required outcomes: complete durable spec/ledger preflight updates, promote `system_tray.rs` from parked test-only module to shipped Windows command path with real Explorer label/icon extraction and stable source-qualified ids, and leave explicit acceptance/readiness gates for phase-2 tray-panel implementation.
- 2026-05-01 `[CODE]` COMPLETED: Phase-0/1 tray plan slice landed. `src-tauri/src/main.rs` now compiles/registers `system_tray` for Windows and exposes `list_system_tray_icons` + `invoke_system_tray_icon` through Tauri. `src-tauri/src/system_tray.rs` now resolves tray labels from Explorer toolbar text (`TBBUTTON::iString` bounded UTF-16 read), attempts native icon extraction from Explorer tray metadata and converts copied `HICON` handles into PNG data URLs, keeps placeholder fallback when extraction fails, and preserves source-qualified snapshot ids. Shared command contracts were synchronized in `src-tauri/src/contracts.rs`, `src/ipc/commands.ts`, and `src/lib/systemTray.ts`.
- 2026-05-01 `[TOOL]` VALIDATED: Ran `cargo test --manifest-path src-tauri\Cargo.toml system_tray` (15 passed, 1 ignored live diagnostic), `node --test tests\systemTray.test.mjs` (5 passed), and `cargo check --manifest-path src-tauri\Cargo.toml` (pass). `npm run test:node` currently fails on unrelated pre-existing search overlay wiring assertion (`tests/overlayDismissalWiring.test.mjs`, expects `<svelte:window on:mousedown={markPanelInteraction}>` in `SearchPanelSurface.svelte`), outside this phase-0/1 tray scope.
- 2026-04-30 `[USER]` REQUESTED/IN_PROGRESS: Fix centered search reopen and input-latency regressions. Required outcomes: after Escape or click-away, clicking the top-bar search field must reopen immediately; Escape must not leave the search cursor active; centered typing must stay non-blocking with background query/update work rather than synchronous UI-thread stalls.
- 2026-04-30 `[CODE]` COMPLETED: Updated centered search to defer `SEARCH_PANEL_QUERY_EVENT` emission through a latest-only zero-delay timer in `src/components/SearchPanelSurface.svelte`, so renderer typing stays immediate while cross-window search work catches up in the background. Updated `src/components/TopBar.svelte` to blur the top-bar search input on close and reopen on search-field pointer interaction even when focus state did not change, fixing the Escape/click-away then immediate-click reopen regression.
- 2026-04-30 `[TOOL]` VALIDATED: Ran `npm run check` (0 errors, 0 warnings), `node --test tests\centeredSearchSurface.test.mjs tests\searchPanelState.test.mjs tests\searchUxState.test.mjs` (46 passed, 0 failed), and `npm run build` (production bundle passed). Production-style startup smoke via `cargo run --manifest-path src-tauri\Cargo.toml --no-default-features --` launched `jason-shell` successfully; smoke instances were closed after startup verification.
- 2026-04-30 `[USER]` REQUESTED/IN_PROGRESS: Fix centered search frontend UX so centered Escape hides immediately, typing in the centered query feels immediate without visible lag, arrow traversal and Enter activation remain intact, and focused regression tests plus focused validation cover the behavior. Expected surfaces: `src/components/SearchPanelSurface.svelte`, centered-search tests, and search-panel state coverage.
- 2026-04-30 `[CODE]` COMPLETED: Updated `src/components/SearchPanelSurface.svelte` so centered search keeps a renderer-local optimistic query draft, stops unconditional refocus on every payload update, and hides the native centered panel immediately on Escape while still forwarding the close intent back to `top-bar` for state synchronization. No `TopBar.svelte` code change was required for this slice.
- 2026-04-30 `[TOOL]` VALIDATED: Final centered-search UX validation ran `npm run check` (0 errors, 0 warnings), `node --test tests\centeredSearchSurface.test.mjs tests\searchPanelState.test.mjs tests\searchUxState.test.mjs` (45 passed, 0 failed), and `npm run build` (production bundle passed). Production-style startup smoke via `cargo run --manifest-path src-tauri\Cargo.toml --no-default-features --` launched `jason-shell` successfully and kept the shell resident until it was closed after startup verification.
- 2026-04-30 `[USER]` REQUESTED/IN_PROGRESS: Re-fix centered search regressions after prior patch did not work live. Required outcomes: centered search input must update results on every query change, ArrowUp/ArrowDown must traverse visible rows, Escape must hide the panel the same way click-away does, regression tests must lock the behavior, and a live smoke test is required.
- 2026-04-30 `[CODE]` COMPLETED: Root cause was unreliable centered search cross-window intent delivery. Updated `src/components/SearchPanelSurface.svelte` to send query/key/select/activate/pin/expand/interact intents directly to `top-bar` via `emitTo(topBarWebviewWindowEventTarget(), ...)`, preserving the existing visible-row keyboard logic and top-bar-owned query/search execution path. Added `tests/centeredSearchSurface.test.mjs` coverage that locks the explicit `top-bar` intent targeting in place.
- 2026-04-30 `[TOOL]` VALIDATED: Ran `npm run check` (0 errors, 0 warnings), `node --test tests\centeredSearchSurface.test.mjs tests\searchPanelState.test.mjs tests\searchUxState.test.mjs` (42 passed, 0 failed), and `npm run build` (production bundle passed). Live smoke: `npm run tauri dev` was blocked by an already-running `node` listener on ports `1420` and `1421`; fallback production-style smoke via `cargo run --manifest-path src-tauri\Cargo.toml --no-default-features --` launched `jason-shell` successfully and kept the desktop app resident until it was closed manually after startup verification.
- 2026-04-30 `[USER]` REQUESTED/IN_PROGRESS: Fix search UI keyboard regressions so active search supports Escape-to-close, ArrowUp/ArrowDown traversal, and Enter activation (apps/folders/files) without requiring mouse double-click.
- 2026-04-30 `[CODE]` COMPLETED: Patched centered search-panel keyboard interaction regressions by removing panel-container focus stealing on interaction in `src/components/SearchPanelSurface.svelte`, adding ArrowUp/ArrowDown/Escape handling on result-row keydown, and preserving query-input keyboard ownership. Updated `src/components/TopBar.svelte` to republish `SearchPanelPayload` when `search-panel:select` updates `selectedIndex`, restoring visible traversal and Enter activation parity with panel rows.
- 2026-04-30 `[TOOL]` VALIDATED: Ran `npm run check` (0 errors, 0 warnings) and `node --test tests\searchPanelState.test.mjs tests\searchUxState.test.mjs` (37 passed, 0 failed).
- 2026-04-30 `[USER]` REQUESTED/IN_PROGRESS: Continue search-upgrade implementation for Phase 5 and Phase 6 from `search_upgrade_plan.md`, satisfy all ordered tasks and acceptance criteria, and execute test-first workflow with validation evidence.
- 2026-04-30 `[CODE]` COMPLETED: Expanded `src-tauri/src/search/providers/settings.rs` phase-6 catalog schema and coverage. Added explicit settings metadata (`keywords`, build guards, optional icon/glyph metadata, safe control-panel args), expanded settings/control-panel entries, and safe `RunControlPanel` argument mapping for subtask rows (`mmsys.cpl`, `appwiz.cpl`). Added phase-6 tests for expanded settings intents, schema safety, build-guard filtering, and safe control-panel argument actions; added scoring assertions for settings-intent precedence (`windows settings`, `control panel`) and Everything-provider acceptance coverage for simple-name runtime behavior.
- 2026-04-30 `[TOOL]` VALIDATED: Ran `cargo test --manifest-path src-tauri\Cargo.toml search::providers::settings` (15/15), `cargo test --manifest-path src-tauri\Cargo.toml search::providers::everything` (13/13), `cargo test --manifest-path src-tauri\Cargo.toml search::scoring` (11/11), `cargo test --manifest-path src-tauri\Cargo.toml search::` (74/74), `cargo check --manifest-path src-tauri\Cargo.toml`, `npx tsc -p tsconfig.test.json`, and `node --test tests\searchUxState.test.mjs tests\searchPanelState.test.mjs tests\searchEngineContracts.test.mjs tests\searchOverhaulPhase0.test.mjs` (59 passed, 0 failed, 3 historical TODO markers).
- 2026-04-30 `[USER]` REQUESTED/IN_PROGRESS: Update search presentation only (no ranking/algorithm changes): render real icons for application/process/file/folder results, and cap dense categories (for example apps/files) to about 7 rows until the user explicitly expands the category.
- 2026-04-30 `[CODE]` COMPLETED: Added cached shell icon extraction for search rows via `src-tauri/src/search/icons.rs` and wired `icon_data_url` population across `apps`, `local`, `everything`, and `open_windows` providers. Extended `SearchOpenWindowContext`/frontend request validation to carry optional `iconDataUrl`, and updated `TopBar.svelte` to forward live task-window icons. Added per-category cap plumbing in `searchUxState` (`DEFAULT_VISIBLE_GROUP_LIMIT = 7`) plus overflow metadata; `SearchPanelSurface.svelte` now shows inline `Show N more <category>` controls and emits `search-panel:expand-group`; `TopBar.svelte` mirrors expanded groups so keyboard navigation matches the visible panel rows.
- 2026-04-30 `[TOOL]` VALIDATED: Ran `npx tsc -p tsconfig.test.json`, `node --test tests\searchUxState.test.mjs tests\searchPanelState.test.mjs tests\searchEngineContracts.test.mjs` (47 passed, 0 failed), `cargo test --manifest-path src-tauri\Cargo.toml search::providers::open_windows` (1/1), `cargo test --manifest-path src-tauri\Cargo.toml search::providers::apps::tests` (10/10), `cargo test --manifest-path src-tauri\Cargo.toml search::providers::local::tests` (3/3), `cargo test --manifest-path src-tauri\Cargo.toml search::providers::everything::tests` (13/13), and `npm run check` (0 errors, 0 warnings).
- 2026-04-30 `[USER]` REQUESTED/IN_PROGRESS: Consolidate duplicate app results in typed search so multiple launchers resolving to the same application (for example multiple Spotify shortcuts) do not bloat visible results.
- 2026-04-30 `[CODE]` COMPLETED: Updated `src-tauri/src/search/providers/apps.rs` app-index dedupe semantics to collapse by canonical launch path instead of raw discovered path. The apps provider now resolves `.lnk` shortcut targets during index build (Windows-only), stores the canonical launch executable path for app actions/record keys, merges aliases across duplicate entries, and keeps one highest-priority row per canonical launch path.
- 2026-04-30 `[TOOL]` VALIDATED: Ran `cargo test --manifest-path src-tauri\Cargo.toml search::providers::apps::tests` (11/11, including new duplicate-collapse coverage) and `npm run check` (0 errors, 0 warnings).
- 2026-04-30 `[USER]` REQUESTED/IN_PROGRESS: Spotify app results still showed duplicate WindowsApps entries and wrong top icon; requested smarter consolidation so best result uses the correct app entry and duplicates are hidden.
- 2026-04-30 `[CODE]` COMPLETED: Strengthened apps-provider dedupe in `src-tauri/src/search/providers/apps.rs` with identity-level collapse (`normalized title + executable stem`) after strict path collapse, plus replacement preference for non-WindowsApps launch paths even when those have lower root priority. Updated shortcut-target resolution to call ShellLink `Resolve` before `GetPath` so canonical launch paths are more accurate for launcher shortcuts. Bumped `APP_INDEX_CACHE_VERSION` to `2` so previously persisted duplicate-heavy app caches are invalidated and rebuilt under the new dedupe behavior.
- 2026-04-30 `[TOOL]` VALIDATED: Ran `cargo test --manifest-path src-tauri\Cargo.toml search::providers::apps::tests` (12/12 with added non-WindowsApps identity-collapse coverage) and `npm run check` (0 errors, 0 warnings).
...
- 2026-05-02T00:00:00-05:00 [USER] REQUESTED/IN_PROGRESS: Start Phase 2 of `multi_fix_plan.md`: Stack Browser rectangle/marquee selection using test-driven development. Expected surfaces: `StackPopupSurface.svelte`, Stack Browser state/helper tests, virtual row selection behavior, row/background pointer wiring, autoscroll, and master_spec.md Stack Browser behavior.
- 2026-05-02T00:00:00-05:00 [CODE] COMPLETED: Implemented Phase 2 Stack Browser rectangle selection. Added pure marquee geometry helpers in `src/lib/stackPopupViewModel.ts`, `selectStackEntryPaths(...)` in `src/lib/stackPopupState.ts`, and Stack Browser pointer-capture marquee wiring/CSS in `src/components/StackPopupSurface.svelte` and `.css`. Marquee starts only from details blank body/virtual spacers, draws a fixed overlay, selects rows intersecting the rectangle using virtual row geometry across autoscroll, supports Ctrl/meta additive selection, preserves row click/double-click/native drag/background context/resize ownership, guards pointer-capture release, and cancels stale drag state on folder changes.
- 2026-05-02T00:00:00-05:00 [TOOL] VALIDATED: RED confirmed first with new marquee tests failing before helper/wiring implementation. GREEN validation passed with `npx tsc -p tsconfig.test.json`, focused `node --test tests\stackPopupViewModel.test.mjs tests\stackPopupState.test.mjs tests\stackPopupMarqueeWiring.test.mjs` (50 passed), broader Stack Browser focused sweep `node --test tests\stackPopupState.test.mjs tests\stackPopupViewModel.test.mjs tests\stackPopupContextMenu.test.mjs tests\stackBrowserContextActions.test.mjs tests\stackBrowserTopBarPinFlow.test.mjs tests\folderDrag.test.mjs tests\stackPopupPagingPhase1Wiring.test.mjs tests\stackPopupPagingPhase2Wiring.test.mjs tests\stackPopupPagingPhase3Wiring.test.mjs tests\stackPopupPagingPhase4Wiring.test.mjs tests\stackPopupPagingPhase5Wiring.test.mjs tests\stackPopupDiagnosticsWiring.test.mjs tests\stackPopupMarqueeWiring.test.mjs` (80 passed), and `npm run check` (0 errors, 0 warnings). Adversarial QA initially BLOCKED on virtual-autoscroll selection dropping unmounted rows; fixed with all-entry virtual geometry and added regression tests. `npm run build` and `npm run validate` were attempted and failed at the Vite build stage in unrelated HTML resolution for `/@id/__x00__virtual:terminal/console` from `index.html` after TypeScript and `svelte-check` passed.

- 2026-05-02T00:00:00-05:00 [USER] REQUESTED/IN_PROGRESS: Investigate and fix typed-search stale results where correct results appear only after pressing trailing Space. Expected surfaces: TopBar search query scheduling/state freshness, search UX helper tests, possible Rust provider cache timing if stale app-cache warmup is root cause.
- 2026-05-02T00:00:00-05:00 [CODE] COMPLETED: Reconciled trailing-Space stale-result fix with a narrower stale-refresh case. `shouldRetrySearchAfterProviderCacheWarm(...)` in `src/features/search/searchUxState.ts` now treats `apps` provider `cache=refresh` as retry-worthy even when `resultCount` is nonzero, because nonzero rows can still be stale. `tests/searchUxState.test.mjs` now covers refresh-with-nonzero-app-rows retry and hit/non-app no-retry cases; `tests/searchAppCacheRefresh.test.mjs` now documents that `cache=hit`, not stale `cache=refresh`, stops retry.
- 2026-05-02T00:00:00-05:00 [TOOL] VALIDATED: Focused search validation passed with `node --test tests/searchTypingFreezePhase1.test.mjs tests/searchAppCacheRefresh.test.mjs tests/searchUxState.test.mjs tests/searchPanelState.test.mjs tests/searchEngineContracts.test.mjs tests/centeredSearchSurface.test.mjs` (69 passed). `npm run check`, `npm run build`, and `cargo test --manifest-path src-tauri\Cargo.toml search::` passed. Full `npm run test:node` was attempted and stopped on an unrelated pre-existing `tests/frontendUiPolicy.test.mjs` Stack Browser source-regex assertion around `listStackFolder(folderPath)`/`commitValidatedStackFolderListing(...)`; search regression tests passed before that unrelated failure.
- 2026-05-02T00:00:00-05:00 [USER] REQUESTED/IN_PROGRESS: Re-investigate fourth trailing-Space typed-search stale app best-match failure after prior retry/cache fixes did not resolve live behavior. Required outcomes: prove whether stale best match is frontend grouping/queue/display or backend provider/ranking/cache, add regression coverage that would have failed the prior implementation, use adversarial-review subagent, implement root fix, validate focused and broad gates. Expected surfaces: TopBar search orchestration, searchUxState grouping/display helpers, search engine contracts, Rust app provider/ranking if implicated, master_spec.md.
- 2026-05-02T12:14:10-05:00 [USER] REQUESTED/IN_PROGRESS: Implementation-worker pass for fourth typed-search stale/wrong best-match failure. Required outcomes: investigate root cause before extending cache retry, add failing no-trailing-space regression where same normalized query rerun fixes stale/wrong best match, keep edits scoped to TopBar/search UX/search engine tests and Rust search only if proven, run focused validation, and preserve unrelated dirty worktree changes.
- 2026-05-02T12:14:10-05:00 [CODE] COMPLETED: Root cause was frontend progressive same-query merge order. Rust progress payloads already contain ranked snapshots, but `nextProgressiveSearchResultSet(...)` merge-appended same-query `local`/`provider` updates into the old working set, preserving stale file/app rows above a fresh app result until a later equivalent rerun such as trailing Space. Updated `src/features/search/searchUxState.ts` so `local`, `provider`, and `complete` progress replace the working set for the payload query. Added `tests/searchUxState.test.mjs` regression that fails when a no-trailing-space same-normalized provider snapshot cannot promote the fresh Spotify app row to best match.
- 2026-05-02T12:14:10-05:00 [TOOL] VALIDATED: RED confirmed with `node --test tests\searchUxState.test.mjs` failing the new same-normalized snapshot regression before the helper fix. GREEN validation passed with `npx tsc -p tsconfig.test.json`, `node --test tests\searchUxState.test.mjs`, focused sweep `node --test tests\searchTypingFreezePhase1.test.mjs tests\searchAppCacheRefresh.test.mjs tests\searchUxState.test.mjs tests\searchPanelState.test.mjs tests\searchEngineContracts.test.mjs` (62 passed), `npm run check` (0 errors, 0 warnings), and `npm run build`.
- 2026-05-02T00:00:00-05:00 [CODE] COMPLETED: Root trailing-Space typed-search fix superseded earlier retry-only attempt. `nextProgressiveSearchResultSet(...)` now treats Rust progress as ranked snapshots instead of same-query deltas; TopBar clears changed-query pending rows, resets default selection to fresh snapshot best match, normalizes sequenced pending queries, cancels queued/retry work on native search-panel close, and only responds to typed app-index refresh generations. `src-tauri/src/search/providers/apps.rs` now reports fresh persisted cache as `cache=refresh` while startup warm is in flight and emits typed apps `search-index:refreshed` payloads after warm completion. `search_panel.rs` now normalizes query identity for same-sequence trailing-space typing/complete payloads.
- 2026-05-02T00:00:00-05:00 [TOOL] VALIDATED: Focused search validation passed: `npx tsc -p tsconfig.test.json`; `node --test tests\searchUxState.test.mjs tests\searchPanelState.test.mjs tests\searchEngineContracts.test.mjs tests\searchAppCacheRefresh.test.mjs tests\searchTypingFreezePhase1.test.mjs tests\centeredSearchSurface.test.mjs` (73 passed); `cargo test --manifest-path src-tauri\Cargo.toml search::providers::apps` (13 passed); `cargo test --manifest-path src-tauri\Cargo.toml search_panel` (10 passed); `cargo test --manifest-path src-tauri\Cargo.toml search::` (77 passed); `npm run check`; `npm run build`. Adversarial-review subagent returned BLOCK on selection/close/global-refresh risks; those findings were fixed before final validation. Full `npm run test:node` still fails on unrelated pre-existing `tests/frontendUiPolicy.test.mjs` Stack Browser regex around `listStackFolder(folderPath)` and `commitValidatedStackFolderListing(...)`; search tests passed in that run before the unrelated failure.

- 2026-05-02T00:00:00-05:00 [USER] REQUESTED/IN_PROGRESS: Investigate and fix typed-search one-character-behind regression where input event search execution appears to use previous query value, causing app best match to appear only after next key or trailing Space. Expected surfaces: TopBar search input handlers, centered search query event path, latest-only execution queue, search UX tests, and master_spec.md search behavior.
- 2026-05-02T00:00:00-05:00 [CODE] COMPLETED: Fixed typed-search one-character-behind risk in `src/components/TopBar.svelte` by changing `queueSearchQueryProcessing(nextQuery)` to enqueue the exact input event value or centered search query-event payload instead of reading `searchInputDraft` inside the queueing function. Added regression coverage for direct top-bar input, centered query events, and rapid `b` -> `br` -> `bra` -> `brav` -> `brave` latest-only coalescing.
- 2026-05-02T00:00:00-05:00 [TOOL] VALIDATED: Ran `npx tsc -p tsconfig.test.json`, `node --test tests\searchTypingFreezePhase1.test.mjs tests\searchPanelState.test.mjs tests\searchUxState.test.mjs tests\searchOverhaulPhase6.test.mjs` (60 passed), and `npm run check` (0 errors, 0 warnings). RED was confirmed first with `node --test tests\searchTypingFreezePhase1.test.mjs` failing the new event-value assertions before the implementation change.
- 2026-05-02T00:00:00-05:00 [USER] REQUESTED/IN_PROGRESS: Follow-up fix for typed-search one-letter-behind live race where old first-letter scheduled or in-flight work can execute during the 16ms deferred-apply window for the next draft query. Expected surfaces: `src/components/TopBar.svelte`, `tests/searchTypingFreezePhase1.test.mjs`, and master search typing contract.
- 2026-05-02T00:00:00-05:00 [CODE] COMPLETED: Strengthened the one-letter-behind fix in `src/components/TopBar.svelte`. `queueSearchQueryProcessing(nextQuery)` now calls `cancelSupersededSearchWorkForDraft(nextQuery)` before enqueueing the latest draft; when the draft differs from the applied query, the helper cancels scheduled provider execution, cancels freshness/cache retry timers, and invalidates existing search-engine responses before the 16ms deferred apply can run.
- 2026-05-02T00:00:00-05:00 [TOOL] VALIDATED: RED confirmed with `node --test tests\searchTypingFreezePhase1.test.mjs` failing the new draft-supersession assertion before the helper existed. GREEN validation passed with `npx tsc -p tsconfig.test.json`, `node --test tests\searchTypingFreezePhase1.test.mjs tests\searchPanelState.test.mjs tests\searchUxState.test.mjs` (55 passed), and `npm run check` (0 errors, 0 warnings).
- 2026-05-02T00:00:00-05:00 [USER] REQUESTED/IN_PROGRESS: Re-investigate fast typed-search live-update failure where queries such as `Windows settings` typed quickly keep showing base/stale results until an extra Space. Required outcomes: prove why visible search state is not updating on every key, add RED tests that fail before the fix for per-character pending/current-query publication, audit frontend/backend search path for easy missed bugs, implement root fix, use focused validation and adversarial QA, and preserve unrelated dirty worktree changes. Expected surfaces: `src/components/TopBar.svelte`, `src/components/SearchPanelSurface.svelte`, search UX/state tests, and Rust search only if backend evidence appears.
- 2026-05-02T00:00:00-05:00 [CODE] COMPLETED: Root cause was frontend batching, not Rust search. `TopBar.svelte` had visible pending payload publication tied to a 16ms latest-only query processor and `SearchPanelSurface.svelte` also coalesced centered query emits through a zero-delay latest-only timer. Fast input could therefore keep stale/base visible rows until an extra key. `TopBar.svelte` now publishes pending changed-query state and starts async search for every exact input/query event with monotonic sequence gates; `SearchPanelSurface.svelte` emits every centered query value directly. Updated `tests/searchTypingFreezePhase1.test.mjs`, `tests/centeredSearchSurface.test.mjs`, and `tests/searchPanelState.test.mjs` to fail on reintroduced 16ms query coalescing, centered query batching, or stale rows during rapid `Windows settings` typing.
- 2026-05-02T00:00:00-05:00 [TOOL] VALIDATED: RED confirmed with `node --test tests\searchTypingFreezePhase1.test.mjs tests\centeredSearchSurface.test.mjs tests\searchPanelState.test.mjs` failing 13 assertions before the final batching removal. GREEN validation passed with `npx tsc -p tsconfig.test.json`, focused `node --test tests\searchTypingFreezePhase1.test.mjs tests\centeredSearchSurface.test.mjs tests\searchPanelState.test.mjs` (43 passed), broader search sweep `node --test tests\searchTypingFreezePhase1.test.mjs tests\centeredSearchSurface.test.mjs tests\searchPanelState.test.mjs tests\searchUxState.test.mjs tests\searchEngineContracts.test.mjs tests\searchAppCacheRefresh.test.mjs tests\searchOverhaulPhase6.test.mjs` (87 passed), and `npm run check` (0 errors, 0 warnings). Backend audit found no Rust provider/ranking root cause for the extra-Space symptom.
- 2026-05-02T00:00:00-05:00 [CODE] COMPLETED: Addressed adversarial-review BLOCK on centered-search out-of-order query events. Added `SearchPanelQueryPayload { query, inputSequence }` plus `isSearchPanelQueryPayload(...)` in `src/lib/searchPanel.ts`; `SearchPanelSurface.svelte` now increments `queryInputSequence` for each centered edit and emits that payload, and `TopBar.svelte` stores `lastSearchPanelInputSequence` and rejects malformed or older centered query payloads before publishing visible state or assigning a search request sequence. Added regression coverage proving an out-of-order `Wi` event cannot overwrite a later `Windows settings` event.
- 2026-05-02T00:00:00-05:00 [TOOL] VALIDATED: Post-review validation passed with source-focused `node --test tests\searchTypingFreezePhase1.test.mjs tests\centeredSearchSurface.test.mjs tests\searchPanelState.test.mjs` (44 passed), `npx tsc -p tsconfig.test.json`, broader search sweep `node --test tests\searchTypingFreezePhase1.test.mjs tests\centeredSearchSurface.test.mjs tests\searchPanelState.test.mjs tests\searchUxState.test.mjs tests\searchEngineContracts.test.mjs tests\searchAppCacheRefresh.test.mjs tests\searchOverhaulPhase6.test.mjs` (88 passed), and `npm run check` (0 errors, 0 warnings).
...
- 2026-05-02T00:00:00-05:00 [USER] REQUESTED/IN_PROGRESS: Continue `multi_fix_plan.md` implementation with strict TDD and phase gates, complete Phase 3 (Bottom quick icons pin/unpin) and Phase 4 (Open folder in VS Code), and finish with QA review cycle.
- 2026-05-02T00:00:00-05:00 [CODE] COMPLETED: Implemented Phase 3 quick icons end-to-end. Added `src-tauri/src/quick_icons.rs` command surface and settings persistence wiring (`quickIcons.entries`) with validation in `src-tauri/src/settings.rs`; added task-window pin action plus quick-icon context menu actions in `src-tauri/src/taskbar_menu.rs`; exposed commands in `src-tauri/src/main.rs` and `src/ipc/commands.ts`; added frontend wrappers (`src/lib/quickIcons.ts`, `src/lib/taskbarMenus.ts`) and bottom-bar rendering/dedupe behavior (`src/components/BottomBar.svelte`, `src/components/BottomBar.css`, `src/lib/taskbarLaunchers.ts` with `targetPath`).
- 2026-05-02T00:00:00-05:00 [CODE] COMPLETED: Implemented Phase 4 folder-in-VS-Code actions. Added shared backend opener/resolver in `src-tauri/src/shell_paths.rs`; added stack command `open_stack_folder_in_vscode` in `src-tauri/src/stack_popup.rs`; added IPC wrapper `openStackFolderInVscode` in `src/lib/stackPopup.ts` and context-menu actions in `src/components/StackPopupSurface.svelte`; extended top-bar pin native menu to include `Open in VS Code` and action dispatch normalization (`open-in-vscode` -> `openInVscode`) via `src-tauri/src/taskbar_menu.rs` and `src/components/TopBar.svelte`.
- 2026-05-02T00:00:00-05:00 [TOOL] VALIDATED: Phase 3/4 focused suites passed: `node --test tests/quickIconsPhase3.test.mjs` (3 passed), `npx tsc -p tsconfig.test.json`, `node --test tests/quickIcons.test.mjs tests/meltMigrationWiring.test.mjs tests/contractsSettings.test.mjs` (15 passed), `cargo test --manifest-path src-tauri/Cargo.toml quick_icons` (4 passed), `cargo test --manifest-path src-tauri/Cargo.toml taskbar_menu::tests` (3 passed), `cargo test --manifest-path src-tauri/Cargo.toml shell_paths::tests` (3 passed), `cargo test --manifest-path src-tauri/Cargo.toml stack_popup::tests::` (33 passed), and `node --test tests/stackPopupContextMenu.test.mjs tests/quickIconsPhase3.test.mjs tests/vscodeFolderOpenPhase4.test.mjs tests/quickIcons.test.mjs` (15 passed). `npm run check` passed. `npm run validate` remains blocked by pre-existing Vite build failure resolving `/@id/__x00__virtual:terminal/console` from `index.html` (unrelated to Phase 3/4 slice).
- 2026-05-03 [USER] REQUESTED/IN_PROGRESS: Complete `launch-and-pin-reliability-P1.md` with strict TDD phase gates so quick-launch failures keep Terminal/Spotify icons and app-managed quick icons can be unpinned without modifying Explorer pins.
- 2026-05-03 [CODE] COMPLETED: Added `tests/quickLaunchReliability.test.mjs`; implemented quick-icon launch failure state helpers in `src/lib/quickIcons.ts`; rendered app-managed quick icons in `src/components/BottomBar.svelte` with `launchQuickIcon`, `showQuickIconContextMenu`, per-icon launch error retention, and unchanged Explorer launcher behavior; renamed native menu labels to `Pin to quick icons` and `Unpin from quick icons`; changed `pin_task_window_quick_icon` to persist settings via `upsert_quick_icon_entry` instead of writing Explorer `.lnk` shortcuts; removed obsolete Explorer taskbar shortcut creation code from `src-tauri/src/launchers.rs`.
- 2026-05-03 [TOOL] VALIDATED: P1 focused gates passed: `npx tsc -p tsconfig.test.json && node --test tests/quickLaunchReliability.test.mjs tests/quickIconsPhase3.test.mjs tests/quickIcons.test.mjs` (11 passed), `cargo test --manifest-path src-tauri/Cargo.toml quick_icon` (4 passed), and `npm run check` (0 errors, 0 warnings). Full `npm run validate` still must be rerun after resolving unrelated legacy source-test expectations from prior P0 popup work.

...
- Shell preferences and Settings panel wiring: `tests/shellPreferences.test.mjs` covers Open Sans default/local asset wiring, preference normalization/application, date/time formatting, storage, BroadcastChannel sync, and listener cleanup. `tests/settingsPanelWiring.test.mjs` covers `settings-panel` surface routing, top-left button show command wiring, settings controls, capability file presence, and Rust placement test presence. `tests/shellBarResize.test.mjs` covers top/bottom resize clamps, drag direction, optimistic height updates, native resize coalescing, stale settings sync protection while resize persistence is pending, Settings lock toggles, and backend resize command wiring.

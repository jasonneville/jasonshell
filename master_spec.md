...
- Primary surfaces: `top-bar`, `bottom-bar`, `task-preview`, `search-panel`, `stack-popup`, `process-manager`, `control-plane`, `settings-panel`, `tray-panel`, `command-panel`, `audio-panel`, and `calendar-panel`, all routed through a shared `index.html` and selected by Tauri webview window label.
...
| `stack-terminal:output` | Rust/app -> `stack-popup` | `StackTerminalOutputChunk` | Emitted while polling process-backed Stack Browser terminal output chunks. |
| `stack-terminal:cwd` | Rust/app -> `stack-popup` | `{ sessionId, cwd }` | Emitted when backend cwd tracking sees `cd`, `chdir`, `Set-Location`, or `sl`. |
| `stack-terminal:closed` | Rust/app -> `stack-popup` | `StackTerminalSessionSnapshot` | Emitted when a terminal session exits or is stopped. |
...
- Stack Browser embedded terminal frontend/backend contracts, toolbar toggle, profile setting, backend command registration, bounded process-backed sessions, cwd-sync wiring, and Rust terminal helpers: `tests/stackBrowserTerminal.test.mjs`; focused Rust coverage: `cargo test --manifest-path src-tauri\Cargo.toml terminal`.

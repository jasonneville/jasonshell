...
test('quick launch protocol source contracts keep camelCase payloads and scoped events', () => {
  ...
  assert.match(quickLaunchRustSource, /emit_to\(QUICK_LAUNCH_PANEL_LABEL, QUICK_LAUNCH_PANEL_OPEN_EVENT/);
  assert.match(quickLaunchRustSource, /emit_to\(BOTTOM_BAR_LABEL, QUICK_LAUNCH_PANEL_CLOSED_EVENT/);
  assert.match(quickLaunchRustSource, /emit_to\(QUICK_LAUNCH_PANEL_LABEL, QUICK_LAUNCH_PANEL_CLOSED_EVENT/);
  assert.match(quickLaunchRustSource, /panel\.hide\(\)\.map_err\(\|error\| format!\("Failed to hide quick launch panel after selection: \{error\}"\)\)\?/);
  assert.match(quickLaunchRustSource, /validate_quick_launch_panel_shortcut\(args\.nonce\.as_str\(\), args\.shortcut_path\.as_str\(\)\)\?/);
  assert.match(quickLaunchRustSource, /allowed_shortcuts\.contains\(&canonical_shortcut_path\)/);
  assert.match(quickLaunchRustSource, /launch_pinned_taskbar_app_internal\(shortcut_path\.to_string_lossy\(\)\.into_owned\(\)\)\?/);
  assert.match(quickLaunchRustSource, /let result = crate::launchers::run_pinned_taskbar_app_as_admin\(shortcut_path\.to_string_lossy\(\)\.into_owned\(\)\);\s+end_quick_launch_panel_focus_hold\(\);\s+result\?;/);
  assert.match(quickLaunchRustSource, /let result = crate::launchers::run_pinned_taskbar_app_as_admin\(shortcut_path\.to_string_lossy\(\)\.into_owned\(\)\);/);
  ...
  assert.match(quickLaunchRustSource, /panel\.hide\(\)\.map_err\(\|error\| format!\("Failed to hide quick launch panel: \{error\}"\)\)\?/);
  assert.match(quickLaunchRustSource, /quick_launch_panel_state_reset\(\);\s+app_handle\.emit_to\(BOTTOM_BAR_LABEL, QUICK_LAUNCH_PANEL_CLOSED_EVENT/);
  assert.match(quickLaunchRustSource, /emit_to\(QUICK_LAUNCH_PANEL_LABEL, QUICK_LAUNCH_PANEL_CLOSED_EVENT/);

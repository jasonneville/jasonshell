...
test('settings panel exposes lock toggles for both shell bars', () => {
  ...
  assert.match(settingsPanelSource, /saveShellSettings\(shellSettings\)/);
});
...
test('top and bottom bars gate visible resize handles behind unlocked settings', () => {
  ...
  assert.match(topBarSource, /saveShellSettings\(shellSettings\)/);
  ...
  assert.match(bottomBarSource, /saveShellSettings\(shellSettings\)/);
});
